...
module firing_state_FSM_2 #(
    ...
    wire w_stp_degree_we, w_stp_coeff_we, w_evp_coeff_re, w_valid_poly, w_rst_all_mem;
    wire [DEGREE_WIDTH-1:0] w_stp_degree_in, w_evp_degree_out;
    wire [3:0] w_stp_coeff_addr, w_evp_coeff_addr;
    wire [COEFF_WIDTH-1:0] w_stp_coeff_in, w_evp_coeff_out;
    wire [ADDR_WIDTH-1:0] w_stp_poly_addr, w_evp_poly_addr;
    ...
    assign w_evp_poly_addr = evp_addr_mux;
    ...
    always @(*) begin
        ...
        l3_stp_start = 0; l3_evp_compute_start = 0; l3_evb_loop_start = 0; l3_rst_start = 0;
        next_state = state;
    ...
    // Dual Compute Engine **
    evp_compute_fsm_3_dual #(.COEFF_WIDTH(COEFF_WIDTH), .DATA_WIDTH(DATA_WIDTH), .DEGREE_WIDTH(DEGREE_WIDTH), .ADDR_WIDTH(ADDR_WIDTH)) 
    evp_inst (
        .clk(clk), .rst(rst), .start(evp_start_mux), .done(l3_evp_compute_done),
        // evp_poly_addr is NOT connected here because the dual module doesn't have it.
        // It is driven by the assign statement above (w_evp_poly_addr = evp_addr_mux).
        .evp_coeff_addr(w_evp_coeff_addr), 
        .evp_coeff_re(w_evp_coeff_re), .evp_degree_out(w_evp_degree_out), 
        .evp_coeff_out(w_evp_coeff_out), .valid_poly(w_valid_poly),
